...
def normalize_phone(text, remove_vow=False):
    regex_pattern = re.compile(r'['+"".join(vowels)+']')
    text = re.sub('[ːˤːٔ ̪̪̤]', '', text)
    # text = re.sub('[ːˤːٔ]', '', text)
    # text = re.sub('d̪', 'd̤', text)
    # text = re.sub('̪̪ ', '̤', text)
    if remove_vow:
...
class PhoneticEditDistance(Levenshtein):
    ...
    def dist_abs_(self, src: str, tar: str, keep_space=False) -> float:
...
def is_mat_token(lang, word):
    if lang == 'ar':
        return isArabic(word)
    if lang == 'te':
        return isTelugu(word)


def isTelugu(word):
    # This function takes a word and checks if it's a Telugu word

    tel_reg = re.compile(r'[\u0C00-\u0C7F]+')
    return (tel_reg.match(word) is not None)
...
def phonetic_sim_dist(phonetic, text_hyp, text_ref, epi_en, epi_mat, matlang, debug=False, keep_space=False):
    """
    ...
    for token in text_hyp.split():
        ...
        if is_mat_token(matlang, token):
            hyp_norm.append(normalize_phone(
                epi_mat.transliterate(token), remove_vow=True))
            hyp_.append(normalize_phone(epi_mat.transliterate(token)))
            ...
            for subtok in re.findall(r"([a-z']+|[^a-z']+)", token):
                if remove_en_characters(subtok) != '':
                    new_token.append(normalize_phone(epi_mat.transliterate(
                        subtok)))
                    new_token_norm.append(normalize_phone(
                        epi_mat.transliterate(subtok), remove_vow=True))
    ...
    for token in text_ref.split():
        ...
        if is_mat_token(matlang, token):
            ref_norm.append(normalize_phone(
                epi_mat.transliterate(token), remove_vow=True))
            ref_.append(normalize_phone(epi_mat.transliterate(token)))
...
def main():
    ...
    results_dir = sys.argv[3]  # results directory
    mat_lang = sys.argv[4]    # matrix language
    ...
    if mat_lang == "ar":
        epi_mat = epitran.Epitran('ara-Arab', ligatures=True)
    elif mat_lang == "te":
        epi_mat = epitran.Epitran('tel-Telu', ligatures=True)

    # read hyp
    for i in range(len(hyp_data)):

        hyp_dict[hyp_data[i].split()[0]] = " ".join(
            hyp_data[i].split()[1:])

    for i in range(len(ref_data)):

        ref_dict[ref_data[i].split()[0]] = " ".join(
            ref_data[i].split()[1:])

    phonetic = PhoneticEditDistance(cost=(1, 1, 8, 0.3))
    hyp_name = os.path.basename(hyp_file).split('.')[0]
    ref_name = os.path.basename(ref_file).split('.')[0]
    results = os.path.join(results_dir, f"results_{hyp_name}_{ref_name}.txt")
    tab_sep_res = os.path.join(
        results_dir, f'tab_sep_res_{hyp_name}_{ref_name}.txt')
    with open(results, "w") as f, open(tab_sep_res, 'w') as b:
        for key in tqdm.tqdm(hyp_dict):
            # print(hyp_dict[key], ref_dict[key])
            # pdb.set_trace()
            metrics, lengths, hyp_phone, ref_phone = phonetic_sim_dist(
                phonetic, hyp_dict[key], ref_dict[key], epi_en, epi_mat, mat_lang, keep_space=True)
        ...
        f.write("\n\n")
        f.write("Mean PER_tot:     " +
                str(round(PER_tot/total_len, 5))+"\n ")
        f.write("Mean PSD_tot:      " +
                str(round(PSD_tot/total_len, 5))+"\n ")
