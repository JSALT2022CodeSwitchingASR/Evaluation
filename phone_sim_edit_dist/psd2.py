...
def phonetic_sim_dist(phonetic, text_hyp, text_ref, epi_en, epi_tar, debug=False):
    """
    ...
    for token in text_hyp.split():
        ...
        if isArabic(token):
            hyp_norm.append(normalize_phone(
                epi_tar.transliterate(token), remove_vow=True))
            hyp_.append(normalize_phone(epi_tar.transliterate(token)))
            ...
            for subtok in re.findall(r"([a-z']+|[^a-z']+)", token):
                if remove_en_characters(subtok) != '':
                    new_token.append(normalize_phone(epi_tar.transliterate(
                        subtok)))
                    new_token_norm.append(normalize_phone(
                        epi_tar.transliterate(subtok), remove_vow=True))
    ...
    for token in text_ref.split():
        ...
        if isArabic(token):
            ref_norm.append(normalize_phone(
                epi_tar.transliterate(token), remove_vow=True))
            ref_.append(normalize_phone(epi_tar.transliterate(token)))
...
def main():
    ...
    tr_lang = sys.argv[3]    # target language
    ...
    if tr_lang == "ar":
        epi_tar = epitran.Epitran('ara-Arab', ligatures=True)
    elif tr_lang == 'te':
        epi_tar = epitran.Epitran('tel-Telu', ligatures=True)
    ...
    with open(f"results_{hyp_name}_{ref_name}.txt", "w") as f, open(f'tab_sep_res_{hyp_name}_{ref_name}.txt', 'w') as b:
        for key in tqdm.tqdm(hyp_dict):
            ...
            metrics, lengths, hyp_phone, ref_phone = phonetic_sim_dist(
                phonetic, hyp_dict[key], ref_dict[key], epi_en, epi_tar)
